...
## Important: If you stop the script before all seeds are finished, this script doesnt know how many seeds are supposed to run, so it will skip even if only 1 seed is finished in a previous run.
...
for MODEL_ROOT_DIR in "${MODEL_ROOT_DIRS[@]}"; do
    ...
    for model_path in $(find $MODEL_ROOT_DIR -type f -name "*curr-ep*"); do
        ...
        # continue if dataset isnt organamnist
        #if [[ $DATASET_NAME != "organamnist" ]]; then
        #    continue
        #fi
