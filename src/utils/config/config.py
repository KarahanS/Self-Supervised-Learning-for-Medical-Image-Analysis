...
class Config:
    ...
    def _sanitize_cfg(self):
        if self.config.Device.num_workers == -1:
            ...
            print(
                f"Number of workers is set to the number of CPU cores ({os.cpu_count()}) since it was set to -1"
            )
        ...
        assert (
            self.config.Dataset.name in DatasetEnum.__members__
        ...
        if self.config.Dataset.name == "medmnist":
            assert (
                self.config.Dataset.params.medmnist_flag in MedMNISTCategory.__members__
        ...
        assert self.config.Dataset.params.image_size in [
            28,
            64,
            128,
            64,
        ], f"Invalid image size: {self.config.Dataset.image_size} (valid sizes: 28, 64, 128, 64)"
        ...
        if "Pretrain" in _train_cfg and "Downstream" in _train_cfg:
            raise ValueError(
                "Both Pretrain and Downstream fields are provided under Training. Please provide only one of them."
            )

        if "Pretrain" in _train_cfg:
            assert (
                _train_cfg.Pretrain.ssl_method in SSLMethod.__members__
            ...
            if isinstance(_train_cfg.Pretrain.augmentations, list):
                raise NotImplementedError(
                    "Custom augmentation sequences are not supported yet."
                )
                # TODO: Assert if the list contains valid torchvision transforms
            else:
                assert (
                    _train_cfg.Pretrain.augmentations
                    in AugmentationSequenceType.__members__
                    or _train_cfg.Pretrain.augmentations in [None, "None"]
            ...
            assert (
                _train_cfg.Pretrain.params.encoder in model_names
            ...
            assert (
                "Downstream" in _train_cfg
            ), "Either Pretrain or Downstream field must be provided."

            assert (
                _train_cfg.Downstream.eval_method in DownstreamMethod.__members__
            ), f"Invalid downstream method: {_train_cfg.Downstream.eval_method} (one of {list(DownstreamMethod.__members__.keys())})"

            assert (
                _train_cfg.Downstream.ssl_method in SSLMethod.__members__
            ...
            if isinstance(_train_cfg.Downstream.augmentations, list):
                ...
                assert (
                    _train_cfg.Downstream.augmentations
                    in AugmentationSequenceType.__members__
                    or _train_cfg.Downstream.augmentations in [None, "None"]
            ...
            model_names = models.list_models()
            assert (
                _train_cfg.Downstream.params.encoder in model_names
        ...
        assert (
            self.config.Logging.tool in LoggingTools.__members__
    ...
    def _cast_cfg(self):
        ...
        self.config.Device.num_workers = self._parse_cfg_str(
            self.config.Device.num_workers, int
        )

        self.config.Dataset.name = DatasetEnum[self.config.Dataset.name]
        self.config.Dataset.params.medmnist_flag = MedMNISTCategory[
            self.config.Dataset.params.medmnist_flag
        ]
        self.config.Dataset.params.image_size = self._parse_cfg_str(
            self.config.Dataset.params.image_size, int
        )

        _train_cfg = self.config.Training
        _train_cfg.params.batch_size = self._parse_cfg_str(
            _train_cfg.params.batch_size, int
        )
        _train_cfg.params.max_epochs = self._parse_cfg_str(
            _train_cfg.params.max_epochs, int
        )
        _train_cfg.params.learning_rate = self._parse_cfg_str(
            _train_cfg.params.learning_rate, float
        )
        _train_cfg.params.weight_decay = self._parse_cfg_str(
            _train_cfg.params.weight_decay, float
        )

        if "Pretrain" in _train_cfg:
            _train_cfg.Pretrain.ssl_method = SSLMethod[_train_cfg.Pretrain.ssl_method]
            _train_cfg.Pretrain.params.hidden_dim = self._parse_cfg_str(
                _train_cfg.Pretrain.params.hidden_dim, int
            )
            _train_cfg.Pretrain.params.output_dim = self._parse_cfg_str(
                _train_cfg.Pretrain.params.output_dim, int
            )
            _train_cfg.Pretrain.params.temperature = self._parse_cfg_str(
                _train_cfg.Pretrain.params.temperature, float
            )
            _train_cfg.Pretrain.params.n_views = self._parse_cfg_str(
                _train_cfg.Pretrain.params.n_views, int
            )

            if isinstance(_train_cfg.Pretrain.augmentations, str):
                _train_cfg.Pretrain.augmentations = AugmentationSequenceType[
                    _train_cfg.Pretrain.augmentations
                ]
        ...
        if "Downstream" in _train_cfg:
            _train_cfg.Downstream.eval_method = DownstreamMethod[
                _train_cfg.Downstream.eval_method
            ]
            _train_cfg.Downstream.ssl_method = SSLMethod[
                _train_cfg.Downstream.ssl_method
            ]
            _train_cfg.Downstream.params.hidden_dim = self._parse_cfg_str(
                _train_cfg.Downstream.params.hidden_dim, int
            )

            if isinstance(self.config.Training.Downstream.augmentations, str):
                self.config.Training.Downstream.augmentations = (
                    AugmentationSequenceType[
                        self.config.Training.Downstream.augmentations
                    ]
                )
            else:  # List[torchvision.transforms]
        ...
        self.config.Logging.log_steps = self._parse_cfg_str(
            self.config.Logging.log_steps, int
        )
    ...
    def __init__(
        ...
        config_path=None,
        config=None,
        defaults_path=(Path(__file__).parent / "defaults.yaml").resolve(),
    ):
        """
        ...
        if config_path:
            ...
            for key in ["Device", "Dataset", "Logging"]:
                if key in self.config:
                    self.config[key] = OmegaConf.merge(
                        self._defaults[key], self.config[key]
                    )
                else:
            ...
            for key in ["params", "checkpoints"]:
                if key in self.config.Training:
                    self.config.Training[key] = OmegaConf.merge(
                        self._defaults.Training[key], self.config.Training[key]
                    )
                else:
            ...
            if "Pretrain" in self.config.Training:
                self.config.Training.Pretrain = OmegaConf.merge(
                    self._defaults.Training.Pretrain, self.config.Training.Pretrain
                )
            elif "Downstream" in self.config.Training:
                self.config.Training.Downstream = OmegaConf.merge(
                    self._defaults.Training.Downstream, self.config.Training.Downstream
                )
            self._sanitize_cfg()
            self._cast_cfg()
        else:
